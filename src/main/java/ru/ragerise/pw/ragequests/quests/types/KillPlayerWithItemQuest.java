...
public class KillPlayerWithItemQuest extends Quest {
    private final Material item;

    public KillPlayerWithItemQuest(int id, String name, List<String> desc, Material icon, Material item, int amount, List<ItemStack> rewards) {
        super(id, name, desc, icon, amount, rewards);
        this.item = item;
    }

    @Override public QuestType getType() { return QuestType.KILL_PLAYER_WITH_ITEM; }

    @Override
    public boolean matchesEvent(org.bukkit.event.Event event, org.bukkit.entity.Player player) {
        if (!(event instanceof EntityDeathEvent e) || !(e.getEntity() instanceof Player) || e.getEntity().getKiller() != player) {
            return false;
        }
